...
# H is shape (25, 25). H[i, j] is count in x_bins[i] and y_bins[j]
# We want to render this in CSS grid.
# CSS grid usually goes top-to-bottom, left-to-right.
# So row 0 is top (highest yield), row 24 is bottom (lowest yield).
# Col 0 is left (lowest drawdown), col 24 is right (highest drawdown).

# H[x, y] where x is drawdown (col), y is yield (row from bottom)
# We need to transpose and flip to match CSS grid (row, col)
# row 0 = highest yield (y=24)

...
export function ReferralInterface() {
  ...
  if (loading) {
    return (
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 animate-pulse">
        {[...Array(4)].map((_, i) => (
          <div key={i} className="h-32 bg-zinc-900/50 rounded-sm border border-zinc-800" />
        ))}
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-6 bg-red-900/20 border border-red-900/50 rounded-sm text-red-400">
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="p-8 bg-zinc-900/50 border border-zinc-800 rounded-sm backdrop-blur-sm relative overflow-hidden group">
        ...
        <div className="relative z-10 space-y-4">
          <div>
            <h2 className=" font-bold text-white mb-2">Your Referral Flywheel</h2>
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            <div className="w-full bg-black border border-zinc-800 rounded-sm px-4 py-3 font-mono text-s text-zinc-300 flex items-center justify-between">
              <span className="truncate mr-4">{stats?.link || 'Connect wallet to generate link'}</span>
              <button 
                onClick={copyToClipboard}
                className="p-2 hover:bg-zinc-800 rounded-sm transition-colors text-zinc-400 hover:text-white"
              >
            ...
            <button 
              onClick={copyToClipboard}
              className="w-full sm:w-auto px-8 py-3 bg-white text-black font-bold rounded-sm hover:bg-zinc-200 transition-all active:scale-95 whitespace-nowrap"
            >
              Copy Link
            </button>
            <div className="flex gap-2">
              <button 
                ...
                className="p-3 bg-[#1DA1F2]/10 text-[#1DA1F2] border border-[#1DA1F2]/20 rounded-sm hover:bg-[#1DA1F2]/20 transition-all"
              ...
              <button 
                ...
                className="p-3 bg-[#0088cc]/10 text-[#0088cc] border border-[#0088cc]/20 rounded-sm hover:bg-[#0088cc]/20 transition-all"
        ...
        <div className="relative group">
          ...
          {stats && parseFloat(stats.pendingCommissions) > 0 && (
            <button 
              onClick={handleClaim}
              disabled={claiming}
              className="absolute top-2 right-2 px-3 py-1 bg-green-500 text-black text-xs font-bold rounded-sm hover:bg-green-400 transition-all disabled:opacity-50"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 p-6 bg-zinc-900/30 border border-zinc-800/50 rounded-sm">
          <h3 className=" font-bold text-white mb-4">Commission Structure</h3>
          <div className="space-y-4">
            <div className="flex items-center justify-between p-4 bg-black/40 rounded-sm border border-zinc-800/30">
        ...
        <div className="p-6 bg-zinc-900/30 border border-zinc-800/50 rounded-sm flex flex-col justify-center items-center text-center h-fit sticky top-24">
          <div className="w-16 h-16 rounded-full bg-zinc-800 flex items-center justify-center mb-4">
            <TrendingUp size={32} className="text-zinc-500" />
          </div>
          <h3 className=" font-bold text-white mb-2">Current Tier</h3>

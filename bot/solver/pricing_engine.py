...
class PricingEngine:
    ...
    def optimize_spread(self):
        ...
        If we are winning too few auctions, we lower the spread.
        If we are winning everything, we raise it to maximize profit.
        """
        ...
        try:
            ...
            if win_rate < 0.2: # Winning less than 20%
            ...
            elif win_rate > 0.6: # Winning more than 60%
    ...
    def calculate_intent_price(self, market_price, funding_rate, hedging_cost, gas_cost_eth=0.0001):
        """
        Calculates the price we can offer the user, now gas-aware.
        """
        ...
        # Profit = (Market - Offer) + Daily_Funding - Hedging_Cost - Gas_Cost
        # Discount_Given = Market - Offer = Market * 0.0001
        discount_bps = 1 
        
        # Convert gas cost to bps (approximate for 1 ETH trade)
        gas_bps = (gas_cost_eth / market_price) * 10000
        
        profit_bps = (daily_funding * 10000) - (hedging_cost * 10000) - discount_bps - gas_bps
        
        is_profitable = profit_bps >= self.current_min_spread_bps
        
        if is_profitable:
            logger.info(f"Pricing Engine: Profitable intent found! Spread: {self.current_min_spread_bps} bps, Expected Profit: {profit_bps:.2f} bps")
        else:

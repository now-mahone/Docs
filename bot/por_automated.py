...
class AutomatedPoRBot:
    ...
    CHAINS = {
        ...
        "arbitrum": {
            ...
            "vault_env": "ARBITRUM_VAULT_ADDRESS",
            "name": "Arbitrum One"
        }
    }
            ...
            if rpc_url and vault_address:
                try:
                    ...
                    if w3.is_connected():
                        ...
                        logger.info(f"Connected to {config['name']}: {vault_address}")
                    else:
                        logger.warning(f"Could not connect to {config['name']}")
                except Exception as e:
                    logger.warning(f"Failed to initialize {chain_key}: {e}")

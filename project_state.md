...
[2026-02-20 14:43] - COINGECKO YIELD INTEGRATION PREPARED: Created a dedicated adapter and API endpoint for CoinGecko Yield submission. This serves as a lower-stakes testing ground before approaching DefiLlama. Added `yield-server/src/adaptors/kerne/index.js` to format Kerne's TVL and APY data. Created `yield-server/src/api/controllers/coingecko.js` and `yield-server/src/api/routes/coingecko.js` to expose the `/coingecko/yield` endpoint matching CoinGecko's required JSON schema. Documented the entire submission process in `docs/guides/COINGECKO_YIELD_SUBMISSION.md`. - Status: COMPLETE
